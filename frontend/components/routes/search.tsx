...
export default function SearchForRoute() {
    const { selected_route } = useQueryParams({ selected_route: { type: "string", default: "", keys: ["r"] } })
    const [searchTerm, setSearchTerm] = useState("")
    const [result, setResult] = useState<RouteSearch[]>([])
    ...
    useEffect(() => {
        const savedSearches = localStorage.getItem('recentRouteSearches')
        if (savedSearches) {
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(async () => {
            if (searchTerm.length >= 2) {
                setIsLoading(true)
                setError(null)
                const data = await searchForRoute(searchTerm)
    ...
    useEffect(() => {
        if (selected_route.found) {
            skipSearchRef.current = true
            setSearchTerm(selected_route.value)
        } else if (searchTerm !== "") {
            setSearchTerm("")
        }
    }, [selected_route])
    ...
    const handleSelectSearch = (item: RouteSearch) => {
        skipSearchRef.current = true
        setSearchTerm(item.name)
        selected_route.set(item.name)
        setIsOpen(false)
...
type searchData =
    ...
    | { error: undefined; result: RouteSearch[] }

async function searchForRoute(search: string): Promise<searchData> {
    if (search.length <= 1) return { error: "Search too short", result: null }
    try {
        const response = await ApiFetch<RouteSearch[]>(
            `routes/find-route/${encodeURIComponent(search)}`
        )
        if (!response.ok) {
